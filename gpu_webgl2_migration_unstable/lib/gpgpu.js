...
/**
 * Compile shader (vertex or fragment) and bind it to a gl program
 */
...
/**
 * Initialize default WebGL program for use in raster rendering.
...
function getDefaultRasterProgram(gl) {
    ...
    compileAndBindShader(gl,program,isGL2
...
function activateVertexShaderPosition(gl,program) {
    ...
    // Re-bind the tracking container for WebGL2 execution loops
    if (gl._rasterVAO) gl.bindVertexArray(gl._rasterVAO);
    ...
    gl.useProgram(null);
    //if (gl._rasterVAO) gl.bindVertexArray(null);
}
/**
...
function buildRasterProgram(gl,fragment,defines) {
    ...
    function callable_program(inputs,output) {
        ...
        //console.log('Caution: GPU program called with one argument.
        // Single input being treated as arguments list.
        // Not as a render target. 
        // If this is not the case, missing variable errors will ensue.');
...
function buildRasterProgram(gl,fragment,defines) {
    ...
    var isGL2 = !!gl.texStorage2D; 
    var source = "";
    if (isGL2) source += "#version 300 es\n";
    source += "precision mediump float;\n";
    if (isGL2) source += "out vec4 fragColor;\n"; 
    defines = defines||{};
    defines['W'] = defines['W'] || gl.width  || 256;
    defines['H'] = defines['H'] || gl.height || 256;
    for (k in defines) if (defines.hasOwnProperty(k))
        source += '#define '+k+' '+asFloat(defines[k])+"\n";
    if (isGL2) {
        fragment = fragment.replace(/gl_FragColor/g, "fragColor");
...
function getRasterProgram(gl,fragment,defines) {
    if ($(fragment)) return buildRasterProgram(gl,$(fragment).text,defines);
...
function bindTexture(gl,program,varname,index,texture) {
    if (index>=8) console.log(
        'Requested texture unit '+index+' but only 0…7 guaranteed to exist.');
    gl.useProgram(program);
...
function newTexture(gl,params) {
    ...
    if ( (H&(H-1))!=0 || (W&(W-1))!=0 ) {
        console.log('Warning: wrapped textures must have power of two size');
        console.log('Ignoring repeat setting and clamping to edge');
        XEDGE = YEDGE = gl.CLAMP_TO_EDGE;
    }
    var isGL2  = !!gl.texStorage2D; 
    if (isGL2 && (params['format']===gl.RGBA32F || params['format']===gl.RGBA16F)) 
        params['mag_filter']=params['min_filter']=gl.NEAREST;
    ...
    // Determine CPU-side data type based on the GPU layout format
    var defaultType = !isGL2? gl.UNSIGNED_BYTE 
                    : params['format']===gl.RGBA32F? gl.FLOAT 
                    : params['format']===gl.RGBA16F? gl.HALF_FLOAT 
                    : gl.UNSIGNED_BYTE
    ...
    gl.texImage2D(target,
        ...
        W,H,0,
        params['outformat'] || gl.RGBA,
        params['outtype'  ] || defaultType,
        params['pixels'   ] || null);
...
function applyProgram(program,inputs,output) {
    ...
    for (var key in inputs) if (inputs.hasOwnProperty(key)) {
        ...
        if (param == null) {
            //console.log('No param '+key+' in shader, unused (compiled out)?');
            continue;
        }
        //console.log(param);
        else if (param.mode === 'texture') {
            //console.log('Parameter '+key+' is texture '+texture_counter);
            var texture = inputs[key];
            // basic raster framebuffer objects enclose their texture
        ...
        else throw new Error("Matrices aren't supported yet");
    }
    if (gl._rasterVAO) gl.bindVertexArray(gl._rasterVAO);
    ...
    var err=gl.getError();
    if (err!==gl.NO_ERROR) console.error("WebGL Draw Error: "+err);
    gl.useProgram(null);
...
/**
 * Get a WebGL 2 context from HTML5 canvas element for raster rendering
 * We're specifically interested in the use case where the GL context
 * Is treated as a raster. So we set the viewport to exactly match
 * the canvas size, and prepare a default geometry that more or less
 * allows us to use only fragment shaders. See also getRasterProgram.
...
function getRasterGL2(canvas) {
    ...
    // If running in WebGL2, ask to write to float buffers
    if (gl instanceof WebGL2RenderingContext) {
        var floatRenderable = gl.getExtension("EXT_color_buffer_float");
        if (!floatRenderable) {
            alert("Hardware does not support writing to Float Textures!");
            return gl;
        }            
    }
    // There are two dimensions we need to worry about
    ...
    // Create and bind a VAO container for WebGL2 configurations
    if (gl.createVertexArray) {
        gl._rasterVAO = gl.createVertexArray();
        gl.bindVertexArray(gl._rasterVAO);
    }
    // Since WebGL is a 2D/3D API, we need to provide a "surface"
    // onto which to draw to directly access pixels. So we make a
    // quadrilateral that fills the whole screen. Combined with
